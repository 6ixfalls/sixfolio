<template>
    <div id="container">
        <div id="textcontainer" class="text-slate-200 break-words">
            <div v-if="shouldShowBlogContent">
        ...
        <div id="comments" class="py-4">
            <Giscus repo="6ixfalls/sixfolio" repoId="R_kgDOH0g_pw" category="Blog" categoryId="DIC_kwDOH0g_p84CXeJP"
                ...
                theme="transparent_dark" lang="en" loading="lazy" v-if="shouldShowComments" />
        </div>
    </div>
</template>
...
let { data: page } = await useAsyncData('current-page', queryContent(route.path).findOne, { watch: [route] });
if (page.value) {
    useSeoMeta({
...
const shouldShowBlogContent = computed(() => {
    return page.value?.layout === 'blog';
});

const shouldShowComments = computed(() => {
    return page.value?.comments_enabled;
});
</script>
