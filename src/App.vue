<template>
    ...
    >
        <div
            ref="topbar"
            class="fixed top-4 left-1/2 -translate-x-1/2 bg-[rgba(255,255,255,0.1)] rounded backdrop-blur-sm border border-[rgba(255,255,255,0.1)] shadow-xl w-[85%] md:w-[40rem] h-12 z-10 flex items-center justify-between select-none"
        >
            <router-link
                class="m-4 font-medium text-white fixed no-underline"
                to="/"
        ...
        >
            ...
            >
                <div
                    ref="name"
                    class="font-bold text-5xl rotate-y-45 translate-z-8"
                >
                    sixfalls
                </div>
                <span
                    ref="desc"
                    class="font-text text-3xl rotate-y-45 translate-z-5"
...
export default defineComponent({
    computed: {
        routes() {
            // only show first level routes
            return this.$router
                .getRoutes()
                .filter(
                    (route) => route.meta.topbar === true && route.meta.friendly
                );
        },
    },
    data() {
